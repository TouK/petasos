const addHeaders = (customHeaders: Record<string, string>) => ({ headers, ...req }: RequestInit) => ({
    ...req,
    headers: {
        ...headers,
        ...customHeaders,
...
export const fetchJson = async <R>(url: string, failIfResponseNotOk: boolean, init:  RequestInit = {}): Promise<R> => {
...
export function withToken(fetchFn: typeof fetchJson, tokenGetter: () => Promise<string>): typeof fetchJson {
