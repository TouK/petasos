...
export const AddGroupDialog = () => {
    ...
    return useObserver(() => {
        ...
        const taskOnSubmit = async (values: GroupFormValues, includeAdvanced: boolean): Promise<void | ValidationError> => {
            if (!values.name) {
        ...
        const initialValues: GroupFormValues = {name: '', advancedValues: undefined}

        return (
            <DialogTemplate<GroupFormValues> advancedFields={() => []} basicFields={basicFields} dialog={dialogs.group}
                                             dialogTitle={"Add new group"} initialValues={initialValues}
                                             onCancel={onCancel} submitButtonText={"Add group"}
                                             onSubmitSuccess={onSubmitSuccess} taskOnSubmit={taskOnSubmit}
                                             validateFunc={validateFunc} wider={false}/>
        )
    })
}
