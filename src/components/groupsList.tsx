...
import { IconButton, List, ListItem, ListItemButton, ListItemText, Stack, styled, Tooltip } from "@mui/material";
    ...
    return (
        <ContentBox>
            <List component="nav" disablePadding>
                <ListItem disablePadding sx={{ color: !selectedGroup && "text.disabled" }}>
                ...
                {groups.names.map((group: string) => (
                    <ListItem key={group} disablePadding sx={{ color: selectedGroup === group && "primary.light" }}>
                            ...
                            <Stack direction="row" ml={2}>
                                ...
                                {groups.isGroupRemoveAllowed && (
                                    <IconButtonWithTooltip
                                        ...
                                        tooltipText={
                                            groups.getTopicsForGroup(group).length > 0 ? "Only empty groups can be deleted" : "Delete group"
                                        }
                                    >
...
const IconButtonWithTooltip = ({ tooltipText, onClick, ...props }: IconButtonWithTooltipProps) => {
    const adjustedButtonProps = props.disabled ? { component: "div" } : { onClick };
    return (
