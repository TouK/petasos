...
export const RetransmitMessageDialog = observer(() => {
  ...
  return (
    <>
      <ConfirmDialog
        dialog={dialog}
        taskOnSubmit={subscription.retransmitMessagesTask.wrap(
          (fn) => () => fn(selectedDate)
        )}
        text={`Are you sure you want to retransmit all messages since ${selectedDate.format(
          timeFormat
        )}?`}
        confirmText={"Retransmit"}
        onSubmitSuccess={async () =>
          subscription.retransmitMessagesTask.reset()
        }
      />
    </>
  );
