...
export const SubscriptionDetails = () => {
    ...
    return useObserver(() => {
        ...
        const [openRetransmissionDialog, setOpenRetransmissionDialog] = useState(
            false);
        ...
        const properties: PropertiesTableRow[] = [
            ...
            createRow(
                "Creation date",
                moment.unix(topics.selectedSubscription.createdAt).
                    format(timeFormat)
            ),
            createRow(
                "Modification date",
                moment.unix(topics.selectedSubscription.modifiedAt).
                    format(timeFormat)
            )
        ];
        ...
        const metrics: PropertiesTableRow[] = topics.selectedSubscription.metrics
            ? [
                topics.selectedSubscription.metrics.rate !== "unavailable" && createRow(
                    "Delivery rate",
                ...
                topics.selectedSubscription.metrics.lag !== "-1" && createRow(
                    "Lag",
                    `${topics.selectedSubscription.metrics.lag}`,
                    SubscriptionInfo.metrics.lag
                ),
                topics.selectedSubscription.metrics.codes2xx !== "unavailable" && createRow(
                    "Codes 2xx",
                    `${topics.selectedSubscription.metrics.codes2xx}`
                ),
                topics.selectedSubscription.metrics.codes4xx !== "unavailable" && createRow(
                    "Codes 4xx",
                    `${topics.selectedSubscription.metrics.codes4xx}`
                ),
                topics.selectedSubscription.metrics.codes5xx !== "unavailable" && createRow(
                    "Codes 5xx",
                    `${topics.selectedSubscription.metrics.codes5xx}`
                ),
                topics.selectedSubscription.metrics.timeouts !== "unavailable" && createRow(
                    "Network timeouts",
                    `${topics.selectedSubscription.metrics.timeouts}`
                ),
                topics.selectedSubscription.metrics.otherErrors !== "unavailable" && createRow(
        ...
        const undeliveredMessage: PropertiesTableRow[] = topics.selectedSubscription.lastUndeliveredMessage
            ? [
                createRow(
                    "Date",
                    moment(topics.selectedSubscription.lastUndeliveredMessage.timestamp).
                        format(timeFormat)
                ),
                createRow(
                    "Reason",
        ...
        const retransmitMessages = async () => {
            await topics.selectedSubscription.retransmitMessagesTask(
                selectedDate);
        ...
        const RetransmitMessageDialog = () =>
            useObserver(() =>
                <StyledDialog open={openRetransmissionDialog}>
                    <DialogContent>
                        <DialogContentText>
                            Are you sure you want to retransmit all messages
                            since
                    ...
                    <DialogActions>
                        ...
                        <LoadingButton color="secondary" variant="contained"
                                       ...
                                       loading={topics.selectedSubscription.retransmitMessagesTask.pending}
                                       text="Retransmit" />
                    </DialogActions>
                </StyledDialog>
            );
                ...
                <StyledDialog open={openStateChangeDialog}>
                    <DialogContent>
                        <DialogContentText>
                            ...
                            to {topics.selectedSubscription.state === "ACTIVE"
                            ? "suspend"
                            : "activate"} subscription <b>{topics.selectedSubscriptionName}</b>?</DialogContentText>
                    </DialogContent>
                    <DialogActions>
                        <Button variant="contained" size="small"
                                onClick={() => setOpenStateChangeDialog(false)}>Cancel</Button>
                        <Button color="secondary" variant="contained"
                                size="small" onClick={changeSubscriptionState}
                    ...
                    <>
                        <div className={styles.DetailsHeader}>
                            <div className={layout.Row}>
                                <div className={layout.Column}>
                                    <div
                                        className={styles.DetailsHeaderSubtitle}>Subscription
                                        to
                                ...
                                <div className={layout.ColumnAlignRight}>
                                    {(topics.selectedSubscription.state === "ACTIVE" || topics.selectedSubscription.state === "SUSPENDED") &&
                                        <>
                                            <Button variant={"contained"}
                                                    color={"primary"}
                                                    startIcon={topics.selectedSubscription.state === "ACTIVE"
                                                        ? <PauseIcon />
                                                        :
                                                        <PlayArrowIcon />}
                                                    onClick={() => setOpenStateChangeDialog(
                                                        true)}>
                                                {topics.selectedSubscription.state === "ACTIVE"
                                                    ? "Suspend"
                                                    : "Activate"}
                                    ...
                                    <Button variant={"contained"}
                                            color={"primary"}
                                            startIcon={<EditIcon />}
                                            onClick={() => dialogs.editSubscription.setOpen(
                                                true)}>Edit</Button>
                                    {" "}<Button variant={"contained"}
                                                 color={"primary"}
                                                 startIcon={<FileCopyIcon />}
                                                 onClick={() => dialogs.addClonedSubscription.setOpen(
                                                     true)}>Clone</Button>
                                    {" "}<Button variant={"contained"}
                                                 color={"primary"}
                                                 startIcon={<DeleteIcon />}
                                                 onClick={() => dialogs.deleteSubscriptionDialog.setOpen(
                                                     true)}>Remove</Button>
                                </div>
                            </div>
                        </div>
                        <div className={styles.DetailsBox}>
                            <div className={styles.DetailsBoxHeader}>Endpoint
                            </div>
                            <StyledPaper style={{
                                padding: "10px",
                                color: "#B4C498"
                                ...
                                ? "Copied!"
                                : "Copy"} placement="top">
                                <SmallIconButton size="small" color="primary"
                                                 type="button"
                        ...
                        <div className={layout.Row}>
                            <div className={layout.Column}>
                                <div className={styles.DetailsBox}>
                                    <div
                                        className={styles.DetailsBoxHeader}>Properties
                                    </div>
                                    <PropertiesTable
                                ...
                                <div className={styles.DetailsBox}>
                                    <div
                                        className={styles.DetailsBoxHeader}>Last
                                        undelivered message
                                    </div>
                                    {topics.selectedSubscription.fetchLastUndeliveredMsgTask.resolved &&
                                        <>
                                            {topics.selectedSubscription.lastUndeliveredMessage
                                                ?
                                                <PropertiesTable
                                                    properties={undeliveredMessage} />
                                                :
                                                <div className={layout.p}>There
                                                    was no undelivered
                                                    message.</div>}
                                        </>}
                            ...
                            <div className={layout.Column}>
                                <div className={styles.DetailsBox}>
                                    <div
                                        className={styles.DetailsBoxHeader}>Metrics
                                    </div>
                                    <div className={layout.Row}>
                                        <div className={layout.Column}>
                                ...
                                <div className={styles.DetailsBox}>
                                    <div
                                        className={styles.DetailsBoxHeader}>Message
                                        retransmission
                                    </div>
                                    <StyledPaper style={{
                                        padding: "10px",
                                        ...
                                        <div style={{marginTop: "10px"}}>
                                            <Button color="secondary"
                                                    variant="contained"
                                                    size="small"
                                                    onClick={() => setOpenRetransmissionDialog(
                                                        true)}>Retransmit</Button>
                                        </div>
