...
export const SubscriptionDetails = observer((props: { topic: Topic; subscription: Subscription }) => {
    ...
    return (
        <>
            {topic && <SubscriptionDetailsHeader topic={topic} subscription={subscription} />}
            <Divider />
...
const SubscriptionMetrics = observer((props: { subscription: Subscription }) => {
    const { subscription } = props;
    ...
    const metrics: PropertiesTableRow[] = subscription.metrics
        ? [
              subscription.metrics.rate !== "unavailable" && createRow("Delivery rate", `${subscription.metrics.rate}`),
              createRow("Delivered messages", `${subscription.metrics.delivered}`),
              createRow("Discarded messages", `${subscription.metrics.discarded}`),
              subscription.metrics.lag !== "-1" && createRow("Lag", `${subscription.metrics.lag}`, SubscriptionInfo.metrics.lag),
              subscription.metrics.codes2xx !== "unavailable" && createRow("Codes 2xx", `${subscription.metrics.codes2xx}`),
              subscription.metrics.codes4xx !== "unavailable" && createRow("Codes 4xx", `${subscription.metrics.codes4xx}`),
              subscription.metrics.codes5xx !== "unavailable" && createRow("Codes 5xx", `${subscription.metrics.codes5xx}`),
              subscription.metrics.timeouts !== "unavailable" && createRow("Network timeouts", `${subscription.metrics.timeouts}`),
              subscription.metrics.otherErrors !== "unavailable" &&
                  createRow("Other network errors", `${subscription.metrics.otherErrors}`),
          ]
        : [];
...
const LastUndeliveredMessage = observer((props: { subscription: Subscription }) => {
    const { subscription } = props;
    ...
    const undeliveredMessage: PropertiesTableRow[] = subscription.lastUndeliveredMessage
        ? [
              createRow("Date", moment(subscription.lastUndeliveredMessage.timestamp).format(timeFormat)),
              createRow("Reason", subscription.lastUndeliveredMessage.reason),
              createRow("Message", <JsonTree jsonText={subscription.lastUndeliveredMessage.message} />),
          ]
        : [];
...
const SubscriptionProperties = observer((props: { subscription: Subscription }) => {
    const { subscription } = props;
    ...
    const advancedProperties: PropertiesTableRow[] = [
        ...
        createRow("Rate limit", `${subscription.subscriptionPolicy.rate}`, SubscriptionInfo.subscriptionPolicy.rate),
        createRow(
            "Sending delay",
