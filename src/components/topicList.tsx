...
import {StyledPagination} from "./styledMuiComponents";
...
export const TopicList = () => {
                                    ...
                                    <div className={topicBarStyles.Topic}>
                                        ...
                                        <div className={topicBarStyles.TopicColumnRight}>
                                            ...
                                            <IconButton size="small" color="primary"
                                                        onClick={() => topics.changeSelectedTopic(topic)}><NavigateNext/></IconButton>
                                        </div>
                                    </div>
                                </CardContent>
                            </StyledTopicCard>)}
    ...
    const TopicsPage = ({topicsList}: { topicsList: string[] }) => useObserver(() => {
                ...
                {topicsList.length === 0 ?
                    <div className={styles.LayoutBodyContent}>
                        ...
                        <div className={layout.LayoutBodyContentInfo}>
                            No topics yet{groups.selectedGroup && ` in group ${groups.selectedGroup}`}.
                        </div>
                    </div> :
                    <>
                        <div className={styles.LayoutBodyContent}>
                            <div className={layout.LayerSectionHeader}>Topics</div>
                            <TopicsSublist
                                topicsList={topicsList.slice((page - 1) * topicsPerPage, Math.min(topicsList.length, page * topicsPerPage))}/>
                        </div>
                        <StyledPagination count={Math.ceil(topicsList.length / topicsPerPage)} page={page}
    ...
    return useObserver(() => {
        return (
            <div className={styles.LayoutBodyMain}>
                {groups.selectedGroup ? <TopicsPage topicsList={topics.getTopicsOfGroup(groups.selectedGroup)}/> :
