...
const validateMaxMessageSize: Validator<TopicFormikValues> =
  (validate) => (values) => {
...
const validateRetentionTime: Validator<TopicFormikValues> =
  (validate) => (values) => {
    const errors = validate?.(values) || {};
...
const validateRequired =
  (
    fields: (keyof Omit<TopicFormikValues, "advancedValues">)[]
  ): Validator<TopicFormikValues> =>
  (validate) =>
  (values) =>
    fields.reduce(
      (errors, field) =>
...
): FormikErrors<TopicFormikValues> {
