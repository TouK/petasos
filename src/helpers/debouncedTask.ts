...
import {
  Task,
  TaskOptions as OriginalTaskOptions,
  WithoutPromise,
} from "mobx-task/lib/task";
...
export function debouncePromise<A extends any[], R extends Promise<any>>(
  innerFn: SomeFn<A, R>,
  ms = 0,
  throttle = false
) {
...
export interface TaskOptions<A extends any[], R>
  extends OriginalTaskOptions<A, R> {
  wait?: number;
  throttle?: boolean;
}
...
export const debouncedTask = debouncedTaskCreatorFactory(
  defaults
) as DebouncedTaskFactory;
