...
export class Groups {
    ...
    @computed get groupOfSelectedTopic(): string {
        if (this.store.topics.selectedTopicName === null) return null;
        const filteredTopics = this.names.filter(group =>
            this.store.topics.selectedTopicName.indexOf(group) === 0
            && this.store.topics.selectedTopicName.split(".").length == group.split(".").length + 1)
        return filteredTopics.length > 0 ? filteredTopics[0] : null;
    }

    @computed get allTopicsList(): string[] {
        let list = []
        this.names.forEach(group => list = list.concat(this.topicsPerGroup.get(group)))
        return list
    }
    ...
    private async addGroup(name: string): Promise<ValidationError | void> {
        ...
        return await fetchFn<ValidationError>(this.url, false,{method: "POST", body})
    }
