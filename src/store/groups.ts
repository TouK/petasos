...
export class Groups {
    ...
    @computed get defaultGroup(): string {
        return this.names.length === 1 ? this.names[0] : this.store.options.forcedGroupName || "";
    }

    @computed get needsForcedGroup() {
        const { forcedGroupName } = this.store.options;
        return this.fetchTask.resolved && forcedGroupName && !this.names.includes(forcedGroupName);
    }
    ...
    @computed get allTopicsList(): string[] {
        let list = [];
        this.names.forEach((group) => (list = list.concat(this.getTopicsForGroup(group))));
        return list;
    }
    ...
    get isGroupAddAllowed() {
        ...
        return !groupsHidden || (!this.fetchTask.pending && (this.groupsMap.size < 1 || this.needsForcedGroup));
    }
        ...
        return fetchFn<string[]>(this.url).then(
            action((data = []) =>
                this.groupsMap.replace(
                    data.reduce(
                        (values, name) => values.set(name, this.groupsMap.get(name) || new Group(this.store, name)),
