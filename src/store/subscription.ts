...
export class Subscription implements SubscriptionModel {
    ...
    fetch100LastUndeliveredMsgsTask = debouncedTask(this.get100LastUndeliveredMessages);
        ...
        if (object.advancedValues) {
            ...
            if (object.advancedValues.backoffMultiplier) {
                ...
                if (object.advancedValues.backoffMaxIntervalInSec) {
                    this.subscriptionPolicy.backoffMaxIntervalInSec = Number(object.advancedValues.backoffMaxIntervalInSec);
                }
            }
            if (object.advancedValues.messageBackoff) {
                this.subscriptionPolicy.messageBackoff = Number(object.advancedValues.messageBackoff);
            }
            if (object.advancedValues.messageTtl) {
                this.subscriptionPolicy.messageTtl = Number(object.advancedValues.messageTtl);
            }
            ...
            if (object.advancedValues.retryClientErrors) {
                this.subscriptionPolicy.retryClientErrors = object.advancedValues.retryClientErrors;
            }
            if (object.advancedValues.sendingDelay) {
                this.subscriptionPolicy.sendingDelay = Number(object.advancedValues.sendingDelay);
            }
        }
    }
    ...
    private async get100LastUndeliveredMessages(): Promise<UndeliveredMessage[]> {
        ...
        const lastUndeliveredMessages = await fetchFn<UndeliveredMessage[]>(undeliveredUrl, { method: "GET" });
