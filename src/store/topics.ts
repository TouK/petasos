...
export class Topics {
    ...
    @computed get selectedTopic(): Topic {
        return this.selectedTopicName === null ? null : (this.topicsMap.get(this.selectedTopicName) || null)
    }

    @computed get selectedSubscription(): Subscription {
        return this.selectedTopicName && this.selectedSubscriptionName && (this.selectedTopic.subscriptionsMap.get(this.selectedSubscriptionName) || null)
    }

    getTopicsOfGroup = computedFn(function getTopicsOfGroup(groupName: string): string[] {
        return this.names ? this.names.filter(name => name.indexOf(groupName + ".") === 0) : []
    })
    ...
    private fetchTopics() {
        ...
        return fetchFn<string[]>(url, true)
            .then(
                action((data) => {
                    this.names = data;
                    data.forEach(topicName =>
                        this.topicsMap.get(topicName) ||
                            this.topicsMap.set(topicName, new Topic(topicName)));
                    [...this.topicsMap.keys()].forEach(topic =>
                        data.includes(topic) || this.topicsMap.delete(topic))
                })
            )
    }

    @action.bound
    private async postTopic(data: Partial<TopicModel>): Promise<ValidationError | void> {
        const url = `${Hosts.APP_API}/topics`
        const fetchFn= fetchJson
        const body = JSON.stringify({...data})
        return await fetchFn<ValidationError | void>(url, false, {method: 'POST', body })
    }
}
