...
export class Topics {
    ...
    forGroup = (groupName: string) => (this.names ? this.names.filter((name) => name.indexOf(`${groupName}.`) === 0) : []);
    ...
    private fetchTopics() {
        ...
        return fetchFn<string[]>(url).then(
            action((data) => {
                this.names = data;
                data.forEach((topicName) => this.getByName(topicName) || this.topicsMap.set(topicName, new Topic(topicName, this.store)));
                [...this.topicsMap.keys()].forEach((topicName) => data.includes(topicName) || this.topicsMap.delete(topicName));
            }),
        );
    }
}
